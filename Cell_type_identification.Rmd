...
load("/gluster/dri02/rdscw/shared/webber/SNatlas_icell8_Novaseq/processed_data/Intron_filtering.RData")
```
...
metadataALL_tmp<-metadataALL_tmp[metadataALL_tmp$Sample_v2!="14_133",] #outlier sample!
...
tmp_cell.anchors <- FindIntegrationAnchors(object.list = tmp_cell.list, anchor.features = features, reduction = "rpca", k.anchor=20)
...
tmp_cell.combined <- RunPCA(tmp_cell.combined, npcs = 30, verbose = FALSE)
tmp_cell.combined <- RunUMAP(tmp_cell.combined, reduction = "pca", dims = 1:30)
tmp_cell.combined <- FindNeighbors(tmp_cell.combined, reduction = "pca", dims = 1:30)
tmp_cell.combined <- FindClusters(tmp_cell.combined, resolution = 0.3)

DimPlot(tmp_cell.combined, reduction = "umap", group.by = "Disease")
