...
chip2_counts<-genesALL_DaN
tmp<-prot_cod[match(rownames(chip2_counts),prot_cod[,1]),]
tmp<-tmp[!duplicated(tmp[,1]),]
chip2_counts_match<-chip2_counts[match(tmp[,1],rownames(chip2_counts)),]
rownames(chip2_counts_match)<-tmp[,1]

chip2_counts_match_step1<-chip2_counts_match/(tmp$Gene_Length/1000)
chip2_counts_match_step1[is.na(chip2_counts_match_step1)]<-0
chip2_counts_match_step2<-colSums(chip2_counts_match_step1)
chip2_counts_match_step3<-chip2_counts_match_step2/1000000
chip2_counts_match_step4 <-chip2_counts_match_step1/chip2_counts_match_step3
SN_TPM<-chip2_counts_match_step4
SN_TPM_log<-log(SN_TPM+1)

SN_TPM_log_red<-SN_TPM_log[,names(pseud_dan)]
...
pheatmap::pheatmap(log(test1+1), cluster_rows=F, cluster_cols=F,scale = "row",fontsize=2)
